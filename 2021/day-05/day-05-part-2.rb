...
def create_line dimensions
    ...
    is_straight = start_point[0] == end_point[0] || start_point[1] == end_point[1]
    is_diagonal = (start_point[0] - end_point[0]).abs == (start_point[1] - end_point[1]).abs

    if start_point[0] < end_point[0] && is_straight
        move = [1,0]
    elsif start_point[0] > end_point[0] && is_straight
        move = [-1,0]
    elsif start_point[1] < end_point[1] && is_straight
        move = [0,1]
    elsif start_point[1] > end_point[1] && is_straight
        move = [0,-1]
    elsif start_point[0] < end_point[0] && start_point[1] < end_point[1] && is_diagonal
        move = [1,1]
    elsif start_point[0] < end_point[0] && start_point[1] > end_point[1] && is_diagonal
        move = [1,-1]
    elsif start_point[0] > end_point[0] && start_point[1] > end_point[1] && is_diagonal
