...
def find_all_paths(cave, current_path, level, path_nodes, all_paths)
    ...
    path_nodes[cave].each do |next_cave|
        to_add = true
        to_add = false if next_cave === "start" || (small_cave?(next_cave) && !first_small_cave?(next_cave, current_path.first(level)))
...
def first_small_cave? cave, visited_caves
    return true if !small_cave?(cave)

    small_caves = visited_caves.select do |cave| 
        small_cave? cave
    end

    unique_small_caves = small_caves.uniq
    duplicate_small_caves = unique_small_caves.select { |cave| small_caves.count(cave) > 1 }

    can_visit = duplicate_small_caves.length === 0
    
    if duplicate_small_caves.length === 1
        can_visit = cave != duplicate_small_caves[0] && !unique_small_caves.include?(cave)
    end
...
def small_cave? cave
    cave === cave.downcase && !["start", "end"].include?(cave)
end
